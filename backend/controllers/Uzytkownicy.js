...
export const getUsers = async (req, res) => {
    try {
        const response = await Uzytkownik.findAll({
...
export const getUserById = async (req, res) => {
    try {
        const response = await Uzytkownik.findOne({
            attributes: ['uuid', 'imie', 'nazwisko', 'email', 'login', 'role'],
            where: {
                uuid: req.params.id
            }
...
export const createUser = async (req, res) => {
    const { imie, nazwisko, email, login, password, confPassword, role } = req.body;
    ...
    try {
        ...
        res.status(201).json({ msg: "Pomyslnie zarejestrowano użytkwonika" });
    } catch (error) {
